...
ただし `homeModules.default` は standalone 利用時に `gitIdentity` と `paths` attrset の注入を前提とします。
private 側では exported module を import したうえで、少なくとも `gitIdentity` と `paths = { homeDir; dotfilesRoot; workspacePath; ghqRootPath; };` を `extraSpecialArgs` などで注入してください。
